 ...
 * Page Component
 * Route: /jobs/:id/edit
 * Edit existing job posting page for companies
 */
      ...
      <main class="container mx-auto px-4 py-8">
        ...
        <div class="mb-8">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-3xl font-bold text-gray-900 mb-2">구인공고 수정</h1>
              <p class="text-gray-600">구인공고 정보를 수정해주세요</p>
        ...
        async function loadJobData() {
          try {
            ...
            if (result.success && result.job) {
              ...
              if (job.requirements) document.getElementById('requirements').value = job.requirements;
              if (job.responsibilities) document.getElementById('responsibilities').value = job.responsibilities;
              if (job.benefits) document.getElementById('benefits').value = job.benefits;
              if (job.salary_min) document.getElementById('salary_min').value = job.salary_min;
              ...
              if (job.visa_types) {
                try {
                  ...
                  console.error('visa_types 파싱 오류:', e);
                }
              }
              ...
              if (job.application_deadline) {
                ...
                if (deadline === '상시모집') {
                  document.querySelector('input[name="deadline_type"][value="always"]').checked = true;
                  document.getElementById('deadline_always_input').classList.remove('hidden');
                } else if (deadline.match(/^\\d{4}-\\d{2}-\\d{2}$/)) {
                  document.querySelector('input[name="deadline_type"][value="date"]').checked = true;
                  document.getElementById('deadline_date').value = deadline;
                  document.getElementById('deadline_date_input').classList.remove('hidden');
                } else {
                  document.querySelector('input[name="deadline_type"][value="text"]').checked = true;
                  document.getElementById('deadline_text').value = deadline;
                  document.getElementById('deadline_text_input').classList.remove('hidden');
                }
              }
              
              console.log('공고 데이터 로드 완료:', job);
            } else {
              alert('공고 정보를 불러올 수 없습니다.');
              window.location.href = '/dashboard/company';
            }
          } catch (error) {
            console.error('공고 데이터 로드 오류:', error);
        ...
        async function submitJobPosting(status) {
          try {
            ...
            if (result.success) {
              ...
              const errorMsg = result.message || '공고 수정에 실패했습니다.';
